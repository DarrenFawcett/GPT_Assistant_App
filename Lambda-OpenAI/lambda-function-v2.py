...
def _is_annual_leave_event(e: dict) -> bool:
    ...
    return any(term in summary for term in LEAVE_TERMS)
...
def _leave_units_for_event(e: dict, tz: str = DEFAULT_TZ) -> float:
    """
    Return how many 'leave days' this event should count for.
    Rules:
      - if summary mentions 'half' -> 0.5
      - if all-day (start.date / end.date) -> number of days spanned (end is exclusive)
      - if timed (start.dateTime / end.dateTime) -> 1.0 unless 'half'
    """
    summary = (e.get("summary") or "").lower()
    if "half" in summary:
        return 0.5
    ...
    # All-day event
    if "date" in start and "date" in end and start["date"] and end["date"]:
        try:
            sd = dt.date.fromisoformat(start["date"])
            ed = dt.date.fromisoformat(end["date"])      # exclusive
            days = max(0, (ed - sd).days)
            return float(days) if days else 1.0
        except Exception:
            return 1.0

    # Timed event -> default to 1 day (unless 'half', handled above)
    return 1.0
    ...
    try:
        if action == "find":
            ...
            for e in leave_events:
                ...
                breakdown.append({
                    "id": e.get("id"),
                    "calendar": e.get("_calendarSummary"),
                    "summary": e.get("summary") or "",
                    "counted_days": units,
                    "start": e.get("start"),
                    "end": e.get("end"),
            ...
            for b in breakdown:
                ...
                try:
                    if "T" in ds:
                        d = datetime.fromisoformat(ds.replace("Z", "+00:00")).astimezone(ZoneInfo(DEFAULT_TZ)).date()
                    else:
                        d = dt.date.fromisoformat(ds)
                    mk = d.strftime("%Y-%m")
                except Exception:
                    pass
                monthly[mk] = monthly.get(mk, 0.0) + b["counted_days"]

            return _resp({"year": year, "total_days": total, "events": breakdown, "by_month": monthly})
