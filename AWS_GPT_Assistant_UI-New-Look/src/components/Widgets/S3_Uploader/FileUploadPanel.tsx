...
export default function FileUploadPanel({
  ...
  const config = {
    files: { label: "Upload Documents", icon: "📄", color: "#10B981", tab: "chat" },
    receipts: { label: "Upload Receipts", icon: "🧾", color: "#F59E0B", tab: "claimtax" },
  }[type];
  ...
  return (
    ...
    >
      ...
      <div
        className="p-5 flex flex-col items-center gap-3 cursor-pointer hover:bg-[rgba(255,255,255,0.02)] transition-all duration-200"
      ...
      >
        ...
        {/* ✅ Dots stay until upload completes */}
        {thinking ? (
          <div className="mt-2 flex justify-center min-h-[32px]">
            <UploadDots color={config.color} />
          </div>
        ) : files.length > 0 ? (
          <div className="flex flex-col w-full max-w-[320px] text-sm gap-1 mt-2">
            {files.map((entry) => (
              <div
